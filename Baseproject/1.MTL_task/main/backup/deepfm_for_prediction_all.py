...
def main():
    ...
    # 获取标准输入
    predictions_8day_sum = []
    predictions_today_sum = []
    target_sum = []
    for i in range(len(chunks)):
        print('=' * 10, '> get chunk ', i + 1)
        ...
        predictions_8day_ = [float(x) for x in predictions_8day]
        predictions_today_ = [float(x) for x in predictions_today]
        test_target_ = [float(x) for x in day1_target]
        predictions_8day_sum = predictions_8day_sum + predictions_8day_
        predictions_today_sum = predictions_today_sum + predictions_today_
        target_sum = target_sum + test_target_
        res_auc_today = roc_auc_score(target_sum, predictions_today_sum)
        print('=' * 10, '> AUC at {}: {}'.format(i + 1, round(res_auc_today, 5)))
    print('=' * 10, '> starting predict')
    res_auc_today = roc_auc_score(target_sum, predictions_today_sum)
    print('=' * 10, '> AUC: {}'.format(round(res_auc_today, 5)))
    predict_log = dict()
