...
from utils import Hash, concat_fun, SparseFeat, DenseFeat, VarLenSparseFeat
...
def create_embedding_matrix(feature_columns, l2_reg, init_std, seed, embedding_size, prefix="",seq_mask_zero=True):
    sparse_feature_columns = list(filter(lambda x: isinstance(x, SparseFeat) and x.embedding, feature_columns)) if feature_columns else []
    ...
    sparse_emb_dict = create_embedding_dict(sparse_feature_columns, varlen_sparse_feature_columns, embedding_size, init_std, seed, l2_reg, prefix=prefix + 'sparse', seq_mask_zero=seq_mask_zero)
...
def get_linear_logit(features, feature_columns, units=1, l2_reg=0, init_std=0.0001, seed=1024, prefix='linear'):
    ...
    if len(dense_input_list) > 0:
        dense_input__ = dense_input_list[0] if len(dense_input_list) == 1 else Concatenate()(dense_input_list)
        linear_dense_logit = Dense(units, activation=None, use_bias=False, kernel_regularizer=l2(l2_reg))(dense_input__)
