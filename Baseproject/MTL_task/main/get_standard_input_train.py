...
def get_standard_input(
    ...
    if use_for_train:
        ...
        data_dir = test_dir
    if use_chunks:
        data = load_data_by_chunks(data_dir, chunkSize=chunk_Size)
        data_size = data.shape
        pid_num = len(data.pid.unique())
        label1_num = dict(zip(data[config.targets[0]].value_counts().index.values, data[config.targets[0]].value_counts().values))
        label2_num = dict(zip(data[config.targets[1]].value_counts().index.values, data[config.targets[1]].value_counts().values))
        print('=' * 10, '> data shape: ', data_size)
        print('=' * 10, '> pid amount: ', pid_num)
        print('=' * 10, '> ' + config.targets[0] + ' nums: ', label1_num)
        print('=' * 10, '> ' + config.targets[1] + ' nums: ', label2_num)
    else:
    ...
    if use_for_train:
        ...
        varlen_feature_columns = [
            VarLenSparseFeat(config.sparse_features_multi_value[i], len(multihot_key2index_lists[i]) + 1,
                             multihot_maxlen_lists[i], 'sum') for i in range(len(config.sparse_features_multi_value))]
        linear_feature_columns = fixlen_feature_columns + varlen_feature_columns
        dnn_feature_columns = fixlen_feature_columns + varlen_feature_columns
        fixlen_feature_names = get_fixlen_feature_names(linear_feature_columns + dnn_feature_columns)
        del fixlen_feature_columns, varlen_feature_columns
        ...
        with open('D:/z_personal_file/DiDi/ycc/ycc/MTL_deepfm/MTL_deepfm/data/dict_data/multihot_dict.json', 'w', encoding='utf-8') as f:
            json.dump(multihot_key2index_lists, f)
        del multihot_key2index_lists, multihot_feature_lists, multihot_maxlen_lists
        gc.collect()
        print('=' * 10, '> check data: \n', data.head(20))
        return data_model_input, [data[config.targets[i]].values for i in range(len(config.targets))], linear_feature_columns, dnn_feature_columns, data_size, pid_num, [label1_num, label2_num]
    else: 
        with open('D:/z_personal_file/DiDi/ycc/ycc/MTL_deepfm/MTL_deepfm/data/dict_data/multihot_dict.json', 'r', encoding='utf-8') as f:
        ...
        del data_multihot
        print('=' * 10, '> predict data: ', data_1day.dt.value_counts())
        data_1day_size = data_1day.shape
        pid_num_1day = len(data_1day.pid.unique())
        label_num_1day = dict(zip(data_1day.is_click.value_counts().index.values, data_1day.is_click.value_counts().values))
        ...
        gc.collect()
        print('=' * 10, '> check data: \n', data.head(20))
        return data_model_input, [data[config.targets[i]].values for i in range(len(config.targets))], data_size, pid_num, label_num, data_1day_model_input, [data_1day[config.targets[i]].values for i in range(len(config.targets))], data_1day_size, pid_num_1day, label_num_1day
