...
def get_standard_data(
    ...
    if use_chunks:
        ...
        #pid_buid[['pid', 'buid']].to_csv('/nfs/project/chuchu/MTL_deepfm/result/' + today + '/pid_buid.csv', index=False, header=None, sep=' ')
        data_size = (label_data.shape[0], chunks[0].shape[1])
        pid_num = len(label_data.pid.unique())
        label1_num = dict(zip(label_data[config.targets[0]].value_counts().index.values, label_data[config.targets[0]].value_counts().values))
        label2_num = dict(zip(label_data[config.targets[1]].value_counts().index.values, label_data[config.targets[1]].value_counts().values))
        #del pid_buid
        gc.collect()
        ...
        print('=' * 10, '> ' + config.targets[0] + ' nums: ', label1_num)
        print('=' * 10, '> ' + config.targets[1] + ' nums: ', label2_num)
    return chunks, data_size, pid_num, [label1_num, label2_num]
...
def get_standard_input(
    ...
    data_multihot = data[config.sparse_features_multi_value + ['dt']]
    ...
    if use_for_train:
        ...
        #print("========>data_model_input",data_model_input)###
        del multihot_feature_lists
        lastday = str(datetime.date.today() - datetime.timedelta(days=1)).replace('-', '')
        data_1day = data[data.dt == 20190725]###
        print("========>data_1day",data_1day)###
        data_multihot_1day = data_multihot[data_multihot.dt == 20190725]##
        ...
        data_1day_model_input = [data_1day[name].values for name in fixlen_feature_names] + \
                                [col for col in multihot_feature_lists]
        del multihot_feature_lists
        #print("=====>data_1day_model_input",data_1day_model_input)##
        #print("=====> [data_1day[config.targets[i]].values for i in range(len(config.targets))", [data_1day[config.targets[i]].values for i in range(len(config.targets))]###
        return data_model_input,  [data[config.targets[i]].values for i in range(len(config.targets))], data_1day_model_input, [data_1day[config.targets[i]].values for i in range(len(config.targets))]
