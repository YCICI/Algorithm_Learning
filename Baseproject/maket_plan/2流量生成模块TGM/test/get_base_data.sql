    ...
    SELECT
        ...
        hash_sub(concat(lower(trim(user_log_acct)),'06')) as month_hash,
        --格子粒度
    ...
    (
        select 
          user_log_acct as pin,
          case when natural_pred_login_cnt<-100 then 0
             else natural_pred_login_cnt end as natural_pred_login_cnt_rest
          
        from 
            app.app_yhzz_umc_a 
        where 
            dt = '2022-03-31' 
            and model = 'groud_truth'
            and priority_type in ('A1','A2')
    )tmp on user.user_log_acct = tmp.pin
