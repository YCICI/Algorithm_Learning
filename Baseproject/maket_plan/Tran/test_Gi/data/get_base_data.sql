...
SELECT
        ...
        COALESCE(grid_tmp.lower_lift_login_cnt,0) as lower_lift_login_cnt,
        ...
        --- 用户各渠道优先级,100分级
        COALESCE(prob.user_class        ,0) as user_class,
        COALESCE(prob.original_class    ,0) as original_class,
        COALESCE(prob.original_class_tg ,0) as original_class_tg,
        COALESCE(prob.original_class_sk ,0) as original_class_sk,
        COALESCE(prob.original_class_jzt,0) as original_class_jzt
    ...
    )grid_tmp on user.grid_name_1st = grid_tmp.grid_name_1st
    ...
    (
        ...
        SELECT
            user_log_acct,
            user_life_cycle_type,
            CAST(user_class_beta AS DOUBLE) AS user_class, --临时处理
            CAST(user_class_tg AS DOUBLE) AS user_class_tg,
            CAST(user_class_sk AS DOUBLE) AS user_class_sk,
            CAST(user_class_jzt AS DOUBLE) AS user_class_jzt,
            CAST(user_class_beta AS DOUBLE) AS user_score,
            CAST(user_score_tg AS DOUBLE) AS user_score_tg,
            CAST(user_score_sk AS DOUBLE) AS user_score_sk,
            CAST(user_score_jzt AS DOUBLE) AS user_score_jzt,
            ntile(100) over(PARTITION BY user_class_beta ORDER BY lift_prob) AS original_class, --在优先级中选择
            --cast(original_class_tg     as double) as  original_class, --天宫的分类（prob的分类暂时作为全渠道优先级的分类）
            CAST(original_class_tg AS DOUBLE) AS original_class_tg,
            CAST(original_class_sk AS DOUBLE) AS original_class_sk,
            CAST(original_class_jzt AS DOUBLE) AS original_class_jzt,
            CAST(lift_prob AS DOUBLE) AS lift_prob,
            CAST(clv AS DOUBLE) AS clv,
            CAST(ltv AS DOUBLE) AS ltv
        FROM
            app.app_yhzz_umc_user_priority
        WHERE
			dt = '{part_dt_3}'

    )prob on user.user_log_acct = prob.user_log_acct
