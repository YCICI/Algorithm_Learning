...
def juge_how_allocation(run_day, plan_level, base_data, this_month_end, priority_type):
    ...
    if priority_type == "('gy_new')":
        if delta_day==0:
            ...
            final_data = base_data.withColumn('result',get_allocation_result('dt','month_clv_threshold','year_clv_threshold','channel_max_adjust',
                                                                            ...
                                                                            'original_class', 'original_class_tg', 'original_class_sk', 'original_class_jzt',
            ...
            lastday_data = get_lastday_data_no_allocation(sql_file='get_lastday_data.sql',
                                                          plan_level = plan_level, 
                                                           ...
                                                           user_type_list = "('gy_new_1','gy_new_2')")
    ...
    if priority_type == "('B1')" or priority_type == "('C')" or priority_type == "('B2')":
        if delta_day==0:
            print(">"*10,"进行首日分配") 
            plan_flag = "firstday"
            final_data = base_data.withColumn('result',get_allocation_result('dt','month_clv_threshold','year_clv_threshold','channel_max_adjust',
...
def get_this_month_end_day(dt):
    ...
    # now_month = now.month
    this_month_end = datetime.datetime.strftime(datetime.datetime(now.year, now.month + 1, 1) - timedelta(seconds=1),'%Y-%m-%d %H:%M:%S')
    this_month_end_date = datetime.datetime.strptime(this_month_end, '%Y-%m-%d %H:%M:%S')
    this_month_end_day = datetime.datetime(this_month_end_date.year, this_month_end_date.month, this_month_end_date.day).strftime('%Y-%m-%d')
...
def BusinessRuleSupplement(basic_result, user_label,  year_clv_threshold, gmv_year, clv, month_clv_threshold, 
    ...
    elif user_label in ('gy_new_1','gy_new_2'):
        if user_label == 'gy_new_1':
            businessrule_plan =  basic_result 
        elif user_label == 'gy_new_2':
            # 非可运营广义新用户
            for channel_name in basic_result:
                businessrule_plan[channel_name] = max(1,businessrule_plan[channel_name])
...
def sampling_supplement(channel_days, adjust_day, user_class):
    ...
    if user_class >= adjust_day_precent_add: # 0.4 TOP40%用户
        channel_days += 1
...
def get_user_channel_max_func(part_dt, priority_type,my_hash_code, channel_businessrule_max_add, channel_businessrule_max, 
                    ...
                    between_channel_adjust, original_class):
    """返回用户各渠道上下限
    Args:
