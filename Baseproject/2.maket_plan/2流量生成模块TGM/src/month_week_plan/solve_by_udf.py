...
def channelcols_to_channeldict(channel_type, col1, col2, col3, col4, col5):
    ...
    if priority_type == "('A1','A2')":
        if delta_day==0:
            ...
            final_data = base_data.withColumn('result',get_allocation_result('dt','month_clv_threshold','year_clv_threshold','channel_max_adjust',
                                                                            ...
                                                                            'com_cnt_rate', 'free_cnt_rate', 'strong_cnt_rate', 'normal_cnt_rate', 'nocontrol_cnt_rate'))
            ...
            final_data = base_data.withColumn('result',get_allocation_result('dt','month_clv_threshold','year_clv_threshold','channel_max_adjust',
                                                                            ...
                                                                            'login_cnt_last_month','natural_login_cnt_last_month','com_login_cnt_last_month','free_login_cnt_last_month','strong_ctrl_login_cnt_last_month','normal_ctrl_login_cnt_last_month','no_ctrl_login_cnt_last_month',
                                                                            'com_login_cnt_mtd','free_login_cnt_mtd','strong_ctrl_login_cnt_mtd','normal_ctrl_login_cnt_mtd','month_hash', 
                                                                            'com_cnt_rate', 'free_cnt_rate', 'strong_cnt_rate', 'normal_cnt_rate', 'nocontrol_cnt_rate','between_channel_adjust' ))
            final_data = add_channel_result_col(final_data, this_month_end)
            
        else:
            print(">"*10,"非首日, 非调控期，不调整") 
            plan_flag = "no_adjust"
            lastday_data = get_lastday_data_no_allocation(plan_level = plan_level, 
                                                           part_dt = run_day,
                                                           part_dt_1day = day1_ago_runday,
                                                           user_type_list= priority_type)
            final_data = lastday_data
    
    if priority_type == "('B1')" or priority_type == "('B1','B2','C')" :
        if delta_day==0:
            print(">"*10,"进行首日分配") 
            plan_flag = "firstday"
            final_data = base_data.withColumn('result',get_allocation_result('dt','month_clv_threshold','year_clv_threshold','channel_max_adjust',
...
def get_allocation_result(part_dt,month_clv_threshold,year_clv_threshold,channel_max_adjust,channel_preference,a1_threshold,priority_type,my_hash_code,
        ...
        # 6月方案临时处理
    #上月取mtd
    if part_dt in ('2022-03-31','2022-04-30','2022-05-31'):
    ...
    # 上月干预次数 乘以大促系数
    if part_dt <= '2022-06-29' and part_dt>='2022-05-31':
        # 大促月才有放大系数
...
def BusinessRuleSupplement(basic_result, user_label,  year_clv_threshold, gmv_year, clv, month_clv_threshold, 
    ...
    print("businessrule_plan['com']",businessrule_plan['com'])
    # businessrule_plan = basic_result
    ...
    if user_label == 'A1':
        ...
        if clv <= month_clv_threshold:
            # clv < 50 保量
            print("clv <= month_clv_threshold clv < 50 保量")
            businessrule_plan = basic_result
            
        else:
            # clv>50, 保量+阶跃
            # 6月新增逻辑
            print("clv > month_clv_threshold clv>50, 保量+阶跃")
            if businessrule_plan['normal_ctrl'] == 0 :
                print("businessrule_plan['normal_ctrl'] == 0 businessrule_plan['normal_ctrl'] +=1")
    ...
    print("businessrule_plan:                  ",businessrule_plan)
    print("businessrule_no_allocation_channel: ",businessrule_no_allocation_channel)
    print("channel_businessrule_max:           ",channel_businessrule_max)
    print("channel_businessrule_max_add:       ",channel_businessrule_max_add)
...
def sampling_supplement(channel_days,adjust_day,my_hash_code):
    ...
    if my_hash_code <= adjust_day_hash_add: # 0.4
        print(" my_hash_code <= adjust_day_hash_add channel_days += 1")
        channel_days += 1
    print("channel_days",channel_days)
    return channel_days
...
def get_user_channel_max_func(part_dt, priority_type, my_hash_code, channel_businessrule_max_add, channel_businessrule_max, 
    ...
    # channel_type = ['com','free','strong_ctrl','normal_ctrl','no_ctrl']
    # channel_businessrule_max_add = ast.literal_eval(channel_businessrule_max_add)
    # channel_businessrule_max = ast.literal_eval(channel_businessrule_max)
    ...
    print("1",channel_max_login_days)
    ...
    if channel_max_adjust == 999:
        ...
        # 随机抽样法调整渠道上限
        if (com_login_cnt_last_month * 1.2 * channel_max_adjust ) > com_max_login_count:
            print("com 1.2*上月>上限 ")
            channel_max_login_days['com'] = com_max_login_count
            print("channel_max_login_days['com']",channel_max_login_days['com'])
        else:
            channel_max_login_days['com'] = sampling_supplement(channel_days = channel_max_login_days['com'],
                                                                adjust_day = com_login_cnt_last_month * 1.2 * channel_max_adjust,
                                                                my_hash_code = my_hash_code)
            print("com 1.2*上月 < 上限 ")
            print("channel_max_login_days['com']",channel_max_login_days['com'])
        # free 上限修正:  1.2*上月>上限，  取上限； 1.2*上月<=上限，取1.2*上月
        if (free_login_cnt_last_month * 1.2 * channel_max_adjust) > free_max_login_count:
            ...
            channel_max_login_days['free'] = sampling_supplement(channel_days = channel_max_login_days['free'] , 
                                                             adjust_day = free_login_cnt_last_month * 1.2 * channel_max_adjust,
                                                             my_hash_code = my_hash_code)
        # strong 上限修正:  1.2*上月>上限，  取上限； 1.2*上月<=上限，取1.2*上月    
        if (strong_ctrl_login_cnt_last_month * 1.2 * channel_max_adjust) > strong_ctrl_max_login_count:
            ...
            channel_max_login_days['strong_ctrl'] = sampling_supplement(channel_days = channel_max_login_days['strong_ctrl'],
                                                            adjust_day = strong_ctrl_login_cnt_last_month * 1.2 * channel_max_adjust,
                                                            my_hash_code = my_hash_code)
        # normal  上限修正:  1.2*上月>上限，取上限； 1.2*上月<=上限，取1.2*上月  
        if (normal_ctrl_login_cnt_last_month * 1.2 * channel_max_adjust) > normal_ctrl_max_login_count:
            ...
            channel_max_login_days['normal_ctrl'] = sampling_supplement(channel_days = channel_max_login_days['normal_ctrl'],
                                                            adjust_day = normal_ctrl_login_cnt_last_month * 1.2 * channel_max_adjust,
                                                            my_hash_code = my_hash_code)
        
    channel_max_login_days['no_ctrl'] = math.ceil(1.0 * no_ctrl_login_cnt_last_month)
    
    print("1.2*上月与上限判断后",channel_max_login_days)
    ...
    print("channel_max_login_days",channel_max_login_days)            
...
def GreedySolver(target_login_days, businessrule_no_allocation_channel, businessrule_result, 
    ...
    print("businessrule_no_allocation_channel:",businessrule_no_allocation_channel)
    print("businessrule_result               :",businessrule_result)
    print("channel_max_login_days            :",channel_max_login_days)
    print("channel_login_costs               :",channel_login_costs)
...
def get_lastday_data_no_allocation(plan_level, part_dt, part_dt_1day, user_type_list):
...
--获取昨日方案
    ...
    """.format(part_dt_1day = part_dt_1day, part_dt = part_dt, part_dp = plan_level, user_type_list = user_type_list)
...
def get_lastday_data_no_allocation(sql_file, plan_level, part_dt, part_dt_1day, user_type_list):
    """
        sql_file (_type_): sql文件路径
        plan_level (_type_): 
        part_dt (_type_): 分区
        part_dt_1day (_type_): 分区-1
        user_type_list (_type_): 用户类型
    """
    
    with open(sql_file, 'r') as f:
        get_lastday_sql = f.read()
        get_lastday_sql = get_lastday_sql.format(part_dt_1day = part_dt_1day, 
                                                 part_dt = part_dt, 
                                                 part_dp = plan_level, 
                                                 user_type_list = user_type_list)
        get_lastday_sql = get_lastday_sql.replace('‘', '\`')
        
    print("get lastday data sql", get_lastday_sql)
    data = spark.sql(get_lastday_sql)
    data.cache()
    print("data columns", data.columns)
    print("data dtypes", data.dtypes)
    return data
