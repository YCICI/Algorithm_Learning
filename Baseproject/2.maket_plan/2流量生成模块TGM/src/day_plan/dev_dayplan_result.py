...
# 渠道月规划
def get_monthplan_data(part_dt):
    used_sql = """
        SELECT
            user_log_acct,
        ...
        FROM
        	(
        		...
        		WHERE
        			dt = '{part_dt}'  --规划值
        			AND dp IN('low')
        			AND SUBSTR(user_log_acct, - 1) IN('u')
        	)
        	a
...
def get_cum_probability(cum_probability_data, rest_day_cnt, day_end):
    ...
    return cum_probability_2


# 日规划：判断某个渠道是否大于'30'次
@udf
def tb_day_plan(natural_visit_cnt_month, com_visit_cnt_month, free_visit_cnt_month, jd_strong_ctrl_visit_cnt_month,
                jd_normal_ctrl_visit_cnt_month, jd_no_ctrl_visit_cnt_month):
    """
    """
    channel_type = ['natural', 'com', 'free', 'jd_strong_ctrl', 'jd_normal_ctrl', 'jd_no_ctrl']
    # 获取剩余月规划值
    channel_month_cnt_dict = {'natural': natural_visit_cnt_month,
                              ...
                              'jd_no_ctrl': jd_no_ctrl_visit_cnt_month
                              }

    print("channel_month_cnt_dict", channel_month_cnt_dict)
    # 初始化
    day_cnt_dict = {'natural': np.zeros(rest_day_cnt),
                    'com': np.zeros(rest_day_cnt),
                    'free': np.zeros(rest_day_cnt),
                    'jd_strong_ctrl': np.zeros(rest_day_cnt),
                    'jd_normal_ctrl': np.zeros(rest_day_cnt),
                    'jd_no_ctrl': np.zeros(rest_day_cnt)
                    }
    ...
    for channel in channel_month_cnt_dict:
        ...
        # 若计划执行首日为0则置为2
        if channel == 'com' or channel == 'jd_normal_ctrl':
    ...
    sql = """
       INSERT OVERWRITE TABLE {table_name} PARTITION (dt='{part_dt}', dp, priority_type)
        ...
        select
            ...
            plan_flag,
            day_cnt,
            split(substr(split(day_cnt, ']')[2], 12), ',')[0] AS natural_today,
            split(substr(split(day_cnt, ']')[0], 7) , ',')[0] AS com_today,
            split(substr(split(day_cnt, ']')[5], 9) , ',')[0] AS free_today,
            split(substr(split(day_cnt, ']')[4], 19), ',')[0] AS jd_strong_today,
            split(substr(split(day_cnt, ']')[3], 19), ',')[0] AS jd_normal_today,
            split(substr(split(day_cnt, ']')[1], 15), ',')[0] AS jd_no_ctrl_today,
            concat(substr(split(day_cnt, ']')[2], 11), ']') AS natural_day_rest,
            concat(substr(split(day_cnt, ']')[0], 6), ']') AS com_day_rest,
            concat(substr(split(day_cnt, ']')[5], 8), ']') AS free_day_rest,
            concat(substr(split(day_cnt, ']')[4], 18), ']') AS jd_strong_day_rest,
            concat(substr(split(day_cnt, ']')[3], 18), ']') AS jd_normal_day_res,
            concat(substr(split(day_cnt, ']')[1], 14), ']') AS jd_no_ctrl_day_res,
            '{part_dp}' dp,
            priority_type
        from
            result_data_tmp     
...
if __name__ == "__main__":
    ...
    # 获取月规划
    month_cnt_data = get_monthplan_data(day_ago_1)
    # 获取渠道同比分布
    ...
    # 生成日规划
    print(">" * 10, "生成日规划")
    month_cnt_data = month_cnt_data.withColumn('day_cnt', tb_day_plan('natural_visit_cnt_month',
                                                     'com_visit_cnt_month',
                                                     'free_visit_cnt_month',
                                                     'jd_strong_ctrl_visit_cnt_month',
                                                     'jd_normal_ctrl_visit_cnt_month',
